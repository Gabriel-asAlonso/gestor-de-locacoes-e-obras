...
import { ConfirmDialog, type ConfirmPrompt } from "./confirm-dialog";
...
export function WorkPartnersPanel({ partners, contributions, openContributionRequest, onContributionRequestConsumed, onPartnersChange, onContributionsChange, onPayment, onNotify, onEvent }: {
  ...
  const removePartner = (partner: WorkPartner) => {
    setConfirmPrompt({
      title: "Remover sócio da distribuição?",
      message: `${partner.name} não participará de novos aportes até ser vinculado novamente. O histórico e os aportes já registrados são preservados.`,
  ...
  return <>
    ...
    {editingPartner !== undefined && <PartnerForm partner={editingPartner} partners={partners} onClose={() => setEditingPartner(undefined)} onSave={savePartner} onRemove={editingPartner ? () => { const target = editingPartner; setEditingPartner(undefined); removePartner(target); } : undefined} />}
...
function PartnerForm({ partner, partners, onClose, onSave, onRemove }: { partner: WorkPartner | null; partners: WorkPartner[]; onClose: () => void; onSave: (partner: WorkPartner) => void; onRemove?: () => void }) {
  const [error, setError] = useState("");
  const othersTotal = participationTotal(partners.filter((item) => item.id !== partner?.id));
  return <ModalPortal><div className="modal-layer" role="dialog" aria-modal="true" aria-label={partner ? "Editar sócio" : "Adicionar sócio"}><button type="button" className="drawer-backdrop" onClick={onClose} aria-label="Fechar formulário" /><form className="receipt-modal work-detail-modal partner-form-modal" onSubmit={(event) => { event.preventDefault(); const data = new FormData(event.currentTarget); const participationPercent = Number(data.get("participationPercent") ?? 0); if (othersTotal + participationPercent > 100) { setError(`A participação total não pode ultrapassar 100%. Há ${100 - othersTotal}% disponíveis.`); return; } onSave({ id: partner?.id ?? nextPartnerRecordId("SOC", partners.map((item) => item.id)), name: String(data.get("name") ?? "").trim(), participationPercent }); }}><header className="modal-header"><div><p className="eyebrow">Distribuição dos novos aportes</p><h2>{partner ? "Editar sócio" : "Adicionar sócio"}</h2></div><button type="button" onClick={onClose} aria-label="Fechar"><X aria-hidden="true" /></button></header><div className="work-detail-modal-body"><div className="form-grid"><p className="work-detail-modal-context full-field"><strong>{100 - othersTotal}% disponíveis</strong>A alteração não modifica a distribuição registrada em aportes anteriores.</p>{error && <p className="form-error-banner full-field" role="alert"><TriangleAlert aria-hidden="true" />{error}</p>}<label className="full-field">Nome do sócio<input name="name" defaultValue={partner?.name ?? ""} placeholder="Pessoa ou empresa participante" required autoFocus /></label><label>Participação nos novos aportes (%)<input name="participationPercent" type="number" min="0.01" max={100 - othersTotal} step="0.01" defaultValue={partner?.participationPercent ?? ""} required onChange={() => setError("")} /></label></div></div><footer className={onRemove ? "partner-form-footer" : undefined}>{onRemove && <button type="button" className="text-button danger partner-form-remove" onClick={onRemove}><Trash2 aria-hidden="true" />Remover sócio</button>}<button type="button" className="secondary-button" onClick={onClose}>Cancelar</button><button type="submit" className="primary-button button-with-icon"><Check aria-hidden="true" />Salvar sócio</button></footer></form></div></ModalPortal>;
}
